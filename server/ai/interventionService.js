...
function addRealUserInterventions(pid, data, items) {
  ...
  if (dayCount > 0) {
    items.push(buildItem(pid, {
      ...
      rationale: `BHI ${stats.healthScore ?? '—'} · 步数 ${stats.steps ?? '—'} · 静息 HR ${stats.restingHR ?? '—'} bpm`,
      rationale_en: `BHI ${stats.healthScore ?? '—'} · steps ${stats.steps ?? '—'} · resting HR ${stats.restingHR ?? '—'} bpm`,
    ...
    cat.items
      ...
      .forEach((item) => {
        ...
        items.push(buildItem(pid, {
          ...
          action: item.recommendation,
          action_en: item.recommendation_en || item.recommendation,
          rationale: `关注信号强度 ${item.evidenceAdjustedAttentionScore ?? item.attentionScore ?? item.calibratedRisk ?? item.risk}%（${item.evidenceLabel || '证据分级 C'}），超出个体基线阈值`,
          rationale_en: `Attention signal ${item.evidenceAdjustedAttentionScore ?? item.attentionScore ?? item.calibratedRisk ?? item.risk}% (${item.evidenceLabel || 'evidence C'}), above personal baseline threshold`,
          aiModel: item.aiModel || 'MedWear-RuleEngine-v1',
  ...
  if (pid === 'real') {
    ...
    if (items.length === 0) {
      ...
      items.push(buildItem(pid, {
        ...
        rationale: `BHI ${stats.healthScore ?? '—'} · 当前为低风险，建议持续监测`,
        rationale_en: `BHI ${stats.healthScore ?? '—'} · low risk — continue monitoring`,
